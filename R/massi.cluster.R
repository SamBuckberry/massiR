...
massi.cluster <- function(y.data){
  
  # import the data from massi.select
  y.data.subset <- y.data

  y.data.subset.t <- as.data.frame(t(y.data.subset)) #transpose the probe subset values matrix

  y.kmedoids <- pam(x=y.data.subset.t, k=2) ### perform clustering of pam.k clusters
  ...
  sample.means <- data.frame(rowMeans(y.data.subset.t)) # calculate the mean of probe values for each sample
  ...
  cluster.1.mean <- mean(cluster.1$rowMeans.y.data.subset.t.) # calculate cluster 1 mean
  
  cluster.2 <- subset(sample.sex, subset=sample.sex$y.kmedoids.clustering==2) # subset cluster 2
  cluster.2.mean <- mean(cluster.2$rowMeans.y.data.subset.t.) # calculate cluster 2 mean
  ...
  massi.results$mean.probe.value <- sample.sex$rowMeans.y.data.subset.t. # add mean probe values
  massi.results<- merge(massi.results, sample.sd, by="ID") # add sample/probe sd
  massi.results$sample.sd <- massi.results$sapply.y.data.subset..1...FUN...sd.

...
massi.cluster.plot <- function(massi.select.data, massi.cluster.data) {
  
  #start new graphics device
  dev.new()
  
  # generate a heatmap of y.chromosome.subset values
  ord <- order(rowSums(abs(massi.select.data)),decreasing=T)
  heatmap.2(x=as.matrix(massi.select.data[ord,]), keysize=2, cexRow=0.7,
  ...
  k.medoids.results <- massi.cluster.data[[1]]
  #y.kmedoids <- get(k.medoids.results)
  clusplot(t(massi.select.data), k.medoids.results$clustering, color=TRUE, shade=FALSE, main="",cex.txt=0.5,
           labels=2, lines=0)

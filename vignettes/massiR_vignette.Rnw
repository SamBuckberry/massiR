...
If the expression data was in the ExpressionSet class at the start of the massiR analysis, you can easily add the sex information to the phenodata of the ExpressionSet by
<<add_to_ExpressionSet>>=
...
male.ids <-
  subset(sample.results$sample_ID,
                   subset=sample.results$sample_sex=="male")

# Get female sample id's
female.ids <-
  subset(sample.results$sample_ID,
         subset=sample.results$sample_sex=="female")
...
To predict the sex of samples for data in an ExpressionSet object using the massiR example data.
...
sexData <- 
  data.frame(eset.results[c("ID", "sex")])
...
all(eset$ID == colnames(exprs(eset)))
@
...
For example, to download the probes corresponding the massiR test dataset, which was obtained from an Illumina v2 array platform and convert these into a format for a massiR analysis:
<<eval=FALSE>>=
library(biomaRt)

# Use biomaRt to download genomic regions for Illumina v2 probes
...
gene.attributes <- 
 getBM(mart=mart, values=TRUE,
       ...
       attributes= c("illumina_humanwg_6_v2", 
                      "chromosome_name", "start_position",
                      "end_position", "strand"))

# remove the probes maspped to multiple genomic regions
unique.probe <- 
